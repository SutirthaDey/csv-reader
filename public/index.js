...
operations.addEventListener('click',async(e)=>{
    if(e.target.value === "read csv data"){
        const response = await axios.get('http://172-31-16-103:3000/readData');
        alert('successful!');
    }
    else if(e.target.value === "get all csv data"){
        try{
            const response = await axios.get('http://172-31-16-103:3000/getAllDetails');
        ...
        try{
            const response = await axios.get('http://172-31-16-103:3000/getSorted');
...
searchByIsbn.addEventListener('submit',async(e)=>{
    try{
        ...
        const response = await axios.get(`http://172-31-16-103:3000/findByIsbn?isbn=${isbn}`);
...
searchByAuthor.addEventListener('submit',async(e)=>{
    try{
        ...
        const response = await axios.get(`http://172-31-16-103:3000/findByAuthor?author=${author}`);
...
addBookMagazine.addEventListener('submit',async(e)=>{
    try{
        ...
        if(e.target.id === 'book-form'){
            ...
            await axios.post('http://172-31-16-103:3000/addBook',bookData);
            alert('book added!');
        }
        else{
            const magazineData = {
            ...
            await axios.post('http://172-31-16-103:3000/addMagazine',magazineData);
